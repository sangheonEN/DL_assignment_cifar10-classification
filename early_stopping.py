...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model, epoch, save_path, log_dir, model_name):
        ...
        if self.best_score is None:
            ...
            self.save_checkpoint(val_loss, model, epoch, save_path, model_name)
            # log file save
            log = [epoch, val_loss]
    ...
    def save_checkpoint(self, val_loss, model, epoch, save_path, model_name):
        """
        ...
        torch.save(model, os.path.join(save_path, f"{model_name}_checkpoint.pth.tar"))
